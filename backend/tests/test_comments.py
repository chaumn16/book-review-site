...
def test_comment_is_posted_and_visible_immediately(client, mock_covers):
...
def test_posting_never_blocks_on_moderation(client, mock_covers):
...
def test_comment_missing_fields_is_rejected(client, mock_covers):
    book = _make_book(client)
    resp = client.post(f"/api/books/{book['id']}/comments", json={"author_name": "", "body": ""})
    assert resp.status_code == 422


def test_comment_rating_is_optional_and_stored(client, mock_covers):
    book = _make_book(client)
    resp = client.post(
        f"/api/books/{book['id']}/comments",
...
def test_comment_rating_out_of_range_is_rejected(client, mock_covers):
    book = _make_book(client)
    resp = client.post(
        f"/api/books/{book['id']}/comments",
