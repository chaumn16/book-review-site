...
def _make_ready_book(title="Dune", author="Frank Herbert"):
    """Insert a fully-generated book directly, bypassing the (now async)
    generation flow -- used by tests that just need an existing 'ready' book
    to attach comments to, not to test generation itself.

    cover_url is set here (not left None) so that a test calling
    /regenerate on this book doesn't trigger regenerate_book()'s "cover
    still missing" fallback and make a real network call.
    """
    db = SessionLocal()
    book = models.Book(
        title=title,
        ...
        verdict_reason="A good one.",
        cover_url="https://covers.example.com/existing.jpg",
    )
    db.add(book)
    db.commit()
    db.refresh(book)
...
def test_a_pending_book_is_not_listed_but_is_reachable_by_id(client, mock_covers):
    created = client.post("/api/books", json={"title": "Dune", "author": "Frank Herbert"}).json()

    assert client.get("/api/books").json() == []
    detail = client.get(f"/api/books/{created['id']}").json()
...
def test_regenerate_queues_a_ready_book_without_touching_its_content(client):
    book_id = _make_ready_book()
    ...
    body = resp.json()
    assert body["status"] == "pending"
    # Old content is left alone until scripts/generate_books.py actually
    # overwrites it -- regenerate only flips the status.
    assert body["summary"] == "A summary."
    assert body["verdict_label"] == "worth_it"


def test_regenerate_not_found(client):
    resp = client.post("/api/books/999/regenerate")
    assert resp.status_code == 404
...
def test_book_list_and_detail_reflect_average_rating(client):
    book_id = _make_ready_book()
...
def test_book_list_comment_count_is_immediate_then_drops_after_moderation(client):
    from app.moderation import review_pending_comments

    book_id = _make_ready_book()
    client.post(f"/api/books/{book_id}/comments", json={"author_name": "Alice", "body": "Loved it!"})
    client.post(f"/api/books/{book_id}/comments", json={"author_name": "Bob", "body": "this has badword in it"})
...
def test_a_removed_comments_rating_does_not_count_toward_the_average(client):
    from app.moderation import review_pending_comments
